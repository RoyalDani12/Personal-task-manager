  ...
  return (
    <>
      ...
      <button 
        ...
        className="lg:hidden fixed top-14 left-4 z-[100] bg-black text-white p-3 rounded-xl shadow-lg active:scale-90 transition-all"
      >
      ...
      `}>
        ...
        <button 
          ...
          className="hidden lg:flex absolute -right-3 top-10 w-6 h-6 bg-black rounded-full items-center justify-center text-white text-[10px] shadow-lg hover:scale-110 transition-transform"
        >
        ...
        <div>
          <div className="flex items-center gap-3 px-6 py-10 mb-4">
            <div className="min-w-[32px] h-8 bg-black rounded flex items-center justify-center text-white font-black italic">
              AX
            </div>
            {(!isCollapsed || isOpen) && (
              <h1 className="text-black text-lg font-bold">
                AXONE.<span className="text-gray-700">PRO</span>
              </h1>
            )}
          </div>
        ...
        {/* USER */}
        <div className={`border-t border-gray-300 p-4 mb-4 flex items-center ${isCollapsed && !isOpen ? "justify-center" : "gap-4"}`}>
          <div className="w-10 h-10 rounded-full border border-gray-300 overflow-hidden">
            {avatar ? (
              ...
              <div className="w-full h-full bg-gray-200 flex items-center justify-center">
                <FontAwesomeIcon icon={faUser} className="text-black" onClick={()=>navigate('/profile-page')} />
              </div>
            )}
          </div>
          ...
          {(!isCollapsed || isOpen) && (
            <div className="overflow-hidden">
              <p className="text-black text-xs font-bold truncate">{UserName}</p>
              <p className="text-gray-700 text-[10px]">Pro Member</p>
            </div>
          )}
        </div>
      </div>
